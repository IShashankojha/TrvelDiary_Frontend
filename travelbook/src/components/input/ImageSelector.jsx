...
const ImageSelector = ({image,setImage,handleDeleteImg}) => {
    ...
    const handleRemoveImage = ()=>{
     ...
     useEffect(() => {
       // if image prop is a string (url) set it as the preview URl
       if(typeof image ==='string'){
        setPreviewUrl(image);
       } else if (image){
        //if the image props is a file object, create a preview file
        setPreviewUrl(URL.createObjectURL(image));
       }else {
        //if there is no image ,clear hte preview url
            setPreviewUrl(null);
       }
